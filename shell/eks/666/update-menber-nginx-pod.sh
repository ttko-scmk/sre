...
#exp
#sh update-mnber-nginx-pod.sh scmk 666-001

rm -rf /root/.kube/
aws eks update-kubeconfig --region ap-east-1 --name dgp-live

if kubectl get deployment.apps/"${v}-${p}-mnber-ng &> /dev/null; then
  echo "Deployment "${v}-${p}-mnber-ng 存在，正在删除..."
  kubectl delete deployment.apps/"${v}-${p}-mnber-ng --force --grace-period=0
  sleep 4
  echo "Deployment "${v}-${p}-mnber-ng 已删除。"
else
  echo "Deployment "${v}-${p}-mnber-ng 不存在，无需删除。"
fi
