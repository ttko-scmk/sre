...
kind: Deployment
metadata:
  name: "${v}-${p}-downodds-java"
  labels:
    app: "${v}-${p}-downodds-java"
spec:
  replicas: 1
    ...
    spec:
      ...
      initContainers:
        - name: init-fetch-s3-log
          ...
          args:
            - |
              SEARCH_KEY=\$(echo "${v}-${p}-downodds-java" | tr -d '-')
              echo "開始搜尋 S3 桶中關鍵字 [${v}-${p}-downodds-java] 及 [\${SEARCH_KEY}] 的歷史 Log..."
              mkdir -p /data/logs

              S3_FILES=\$(aws s3 api list-objects-v2 --bucket kklo-logs --query "Contents[?contains(Key, '\${SEARCH_KEY}') || contains(Key, '${v}-${p}-downodds-java')].Key" --output text --region ap-east-1 2>/dev/null || true)
      ...
      containers:
        # 主服務容器
        - name: "${v}-${p}-downodds-java"
          image: public.ecr.aws/f2z7x9a1/ttko-666:springboot-downodds-god
