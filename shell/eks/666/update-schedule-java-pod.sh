...
if kubectl get deployment.apps/"${v}-${p}-schedule-java" &> /dev/null; then
  echo "Deployment ${v}-${p}-schedule-java 存在，正在刪除..."
  kubectl delete deployment.apps/"${v}-${p}-schedule-java" --force --grace-period=0
  sleep 4
  echo "Deployment ${v}-${p}-schedule-java 已刪除。"
else
  echo "Deployment ${v}-${p}-schedule-java 不存在，無需刪除。"
fi
...
kind: Deployment
metadata:
  name: "${v}-${p}-schedule-java"
  labels:
    app: "${v}-${p}-schedule-java"
spec:
  replicas: 1
    ...
    spec:
      ...
      initContainers:
        - name: init-fetch-s3-log
          ...
          args:
            - |
              SEARCH_KEY=\$(echo "${v}-${p}-schedule-java" | tr -d '-')
              echo "開始搜尋 S3 桶中關鍵字 [${v}-${p}-schedule-java] 及 [\${SEARCH_KEY}] 的歷史 Log..."
              mkdir -p /data/logs

              S3_FILES=\$(aws s3 api list-objects-v2 --bucket kklo-logs --query "Contents[?contains(Key, '\${SEARCH_KEY}') || contains(Key, '${v}-${p}-schedule-java')].Key" --output text --region ap-east-1 2>/dev/null || true)
      ...
      containers:
        # 主服務容器
        - name: "${v}-${p}-schedule-java"
          image: public.ecr.aws/f2z7x9a1/ttko-666:springboot-schedule-god
