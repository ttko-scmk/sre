...
if kubectl get deployment.apps/"${v}-${p}-settlement-java" &> /dev/null; then
  echo "Deployment ${v}-${p}-settlement-java 存在，正在刪除..."
  kubectl delete deployment.apps/"${v}-${p}-settlement-java" --force --grace-period=0
  sleep 4
  echo "Deployment ${v}-${p}-settlement-java 已刪除。"
else
  echo "Deployment ${v}-${p}-settlement-java 不存在，無需刪除。"
fi
...
kind: Deployment
metadata:
  name: "${v}-${p}-settlement-java"
  labels:
    app: "${v}-${p}-settlement-java"
spec:
  replicas: 1
    ...
    spec:
      ...
      topologySpreadConstraints:
        - maxSkew: 1
          ...
          whenUnsatisfiable: settlementAnyway
          labelSelector:
            matchLabels:
              app: "${v}-${p}-settlement-java"
      nodeSelector:
      ...
      initContainers:
        - name: init-fetch-s3-log
          ...
          args:
            - |
              SEARCH_KEY=\$(echo "${v}-${p}-settlement-java" | tr -d '-')
              echo "開始搜尋 S3 桶中關鍵字 [${v}-${p}-settlement-java] 及 [\${SEARCH_KEY}] 的歷史 Log..."
              mkdir -p /data/logs

              S3_FILES=\$(aws s3 api list-objects-v2 --bucket kklo-logs --query "Contents[?contains(Key, '\${SEARCH_KEY}') || contains(Key, '${v}-${p}-settlement-java')].Key" --output text --region ap-east-1 2>/dev/null || true)
      ...
      containers:
        # 主服務容器
        - name: "${v}-${p}-settlement-java"
          image: public.ecr.aws/f2z7x9a1/ttko-666:springboot-settlement-god
