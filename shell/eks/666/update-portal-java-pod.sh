...
# exp
# sh update-portal-java-pod.sh rfhgrt 666-01 scmk02-live.cluster-crq0wu4woeyc.ap-east-1.rds.amazonaws.com scmk-live-instance-1.crq0wu4woeyc.ap-east-1.rds.amazonaws.com:3306/scmk 10.7.200.10 logstash.taidagediao.com:4582

rm -rf /root/.kube/
aws eks update-kubeconfig --region ap-east-1 --name dgp-live

if kubectl get deployment.apps/"${v}-${p}-portal-java" &> /dev/null; then
  echo "Deployment ${v}-${p}-portal-java 存在，正在刪除..."
  kubectl delete deployment.apps/"${v}-${p}-portal-java" --force --grace-period=0
  sleep 4
  echo "Deployment ${v}-${p}-portal-java 已刪除。"
else
  echo "Deployment ${v}-${p}-portal-java 不存在，無需刪除。"
fi
    ...
    spec:
      ...
      initContainers:
        - name: init-fetch-s3-log
          ...
          args:
            - |
              SEARCH_KEY=\$(echo "${v}-${p}-portal-java" | tr -d '-')
              echo "開始搜尋 S3 桶中關鍵字 [${v}-${p}-portal-java] 及 [\${SEARCH_KEY}] 的歷史 Log..."
              mkdir -p /data/logs

              S3_FILES=\$(aws s3 api list-objects-v2 --bucket kklo-logs --query "Contents[?contains(Key, '\${SEARCH_KEY}') || contains(Key, '${v}-${p}-portal-java')].Key" --output text --region ap-east-1 2>/dev/null || true)
      ...
      containers:
        # 主服務容器
        - name: "${v}-${p}-portal-java"
          image: public.ecr.aws/f2z7x9a1/ttko-666:springboot-portal-god
...
rm -rf 123.yaml
kubectl rollout status deployment.apps/"${v}-${p}-portal-java"
