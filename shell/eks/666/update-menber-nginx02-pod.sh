...
#exp
#sh update-mnber-ng02inx-pod.sh scmk 666-001

rm -rf /root/.kube/
aws eks update-kubeconfig --region ap-east-1 --name dgp-live

if kubectl get deployment.apps/"${v}-${p}-mnber-ng02 &> /dev/null; then
  echo "Deployment "${v}-${p}-mnber-ng02 存在，正在删除..."
  kubectl delete deployment.apps/"${v}-${p}-mnber-ng02 --force --grace-period=0
  sleep 4
  echo "Deployment "${v}-${p}-mnber-ng02 已删除。"
else
  echo "Deployment "${v}-${p}-mnber-ng02 不存在，无需删除。"
fi
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: "${v}-${p}-mnber-ng02
          image: public.ecr.aws/f2z7x9a1/ttko-666:spring02boot-mnber-ng-god
